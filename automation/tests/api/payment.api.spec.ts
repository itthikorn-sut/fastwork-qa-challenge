...
import {
  VALID_CARD,
  VALID_CARD_MC, VALID_CARD_JCB, VALID_CARD_AMEX, VALID_CARD_UNIONPAY,
  DECLINED_CARD,
  CARD_INSUFFICIENT_FUND, CARD_STOLEN_OR_LOST, CARD_FAILED_PROCESSING,
  CARD_PAYMENT_REJECTED, CARD_FAILED_FRAUD_CHECK, CARD_INVALID_ACCOUNT,
  CARD_3DS_ENROLLMENT_FAIL, CARD_3DS_VALIDATION_FAIL,
...
// ─────────────────────────────────────────────────────────────────────────────
// Multi-brand success cards (Omise test cards — docs.omise.co/api-testing)
// ─────────────────────────────────────────────────────────────────────────────
test.describe('API — Multi-brand success cards (Omise)', () => {

  for (const [brand, card] of [
    ['Mastercard', VALID_CARD_MC],
    ['JCB',        VALID_CARD_JCB],
    ['Amex',       VALID_CARD_AMEX],
    ['UnionPay',   VALID_CARD_UNIONPAY],
  ] as const) {
    test(`${brand} success card returns 200`, async ({ request }) => {
      const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
      const res = await request.post('/api/v1/payments', {
        data: {
          quotation_id: quotationId,
          credit_card_number: card.number,
          credit_card_owner_name: card.ownerName,
...
// ─────────────────────────────────────────────────────────────────────────────
// Decline scenarios by Omise failure_code
// ─────────────────────────────────────────────────────────────────────────────
test.describe('API — Omise decline scenarios by failure_code', () => {

  for (const [label, card, expectedCode] of [
    ['stolen_or_lost_card',    CARD_STOLEN_OR_LOST,     'stolen_or_lost_card'],
    ['failed_processing',      CARD_FAILED_PROCESSING,  'failed_processing'],
    ['payment_rejected',       CARD_PAYMENT_REJECTED,   'payment_rejected'],
    ['failed_fraud_check',     CARD_FAILED_FRAUD_CHECK, 'failed_fraud_check'],
    ['invalid_account_number', CARD_INVALID_ACCOUNT,    'invalid_account_number'],
  ] as const) {
    test(`returns 402 with failure_code "${label}"`, async ({ request }) => {
      const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
      const res = await request.post('/api/v1/payments', {
        data: validPayload(quotationId, totalAmount, { credit_card_number: card.number }),
      });
      expect(res.status()).toBe(402);
      expect((await res.json()).failure_code).toBe(expectedCode);
    });
  }

  test('MC insufficient_fund card returns 402 with correct failure_code', async ({ request }) => {
    const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
    const res = await request.post('/api/v1/payments', {
      data: validPayload(quotationId, totalAmount, { credit_card_number: '5555551111110011' }),
    });
    expect(res.status()).toBe(402);
    expect((await res.json()).failure_code).toBe('insufficient_fund');
  });

  test('UnionPay stolen_or_lost card returns 402 with correct failure_code', async ({ request }) => {
    const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
    const res = await request.post('/api/v1/payments', {
      data: validPayload(quotationId, totalAmount, { credit_card_number: '6250947000000022' }),
    });
    expect(res.status()).toBe(402);
    expect((await res.json()).failure_code).toBe('stolen_or_lost_card');
  });
});

// ─────────────────────────────────────────────────────────────────────────────
// 3DS failure scenarios (Omise test cards)
// ─────────────────────────────────────────────────────────────────────────────
test.describe('API — 3DS failure scenarios (Omise)', () => {

  test('3DS enrollment failure returns 402 with failure_code "3ds_enrollment_failure"', async ({ request }) => {
    const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
    const res = await request.post('/api/v1/payments', {
      data: validPayload(quotationId, totalAmount, { credit_card_number: CARD_3DS_ENROLLMENT_FAIL.number }),
    });
    expect(res.status()).toBe(402);
    expect((await res.json()).failure_code).toBe('3ds_enrollment_failure');
  });

  test('3DS validation failure returns 402 with failure_code "3ds_validation_failure"', async ({ request }) => {
    const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
    const res = await request.post('/api/v1/payments', {
      data: validPayload(quotationId, totalAmount, { credit_card_number: CARD_3DS_VALIDATION_FAIL.number }),
    });
    expect(res.status()).toBe(402);
    expect((await res.json()).failure_code).toBe('3ds_validation_failure');
  });

  test('Amex 3DS validation failure returns 402 with correct failure_code', async ({ request }) => {
    const { quotationId, totalAmount } = await createAndAcceptQuotation(request);
    const res = await request.post('/api/v1/payments', {
      data: validPayload(quotationId, totalAmount, { credit_card_number: '377138161111003' }),
    });
    expect(res.status()).toBe(402);
    expect((await res.json()).failure_code).toBe('3ds_validation_failure');
  });
